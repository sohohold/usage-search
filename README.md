...
Turso には上書き・置き換えの操作がありません（CLI にもプラットフォーム API にもありません）。
同名にする唯一の方法は destroy してから作り直すことで、その時点で既存の認証トークンは無効になります。
そのため既定では実行のたびに別名（分まで含む）のデータベースを作ります。
切り替えを確認したら、旧データベースは `turso db destroy <旧名> --yes` で削除してください。

同名で作り直したい場合は `replace` を有効にしてください。既存のデータベースを破棄してから作成します。
破棄した時点でそのデータベースのトークンは無効になるため、本番が参照しているデータベースには使わないでください。
