...
> **既存インデックスをお持ちの場合**: 本文整形の不具合（入力者注 `［＃…］` の残留、凡例ブロックの混入、
> 長大段落の未分割）を修正したため、既存のインデックスは作り直しが必要です。詳細は
> [テスト仕様書 §9](./docs/test-spec.md#9-設計判断の記録) の D-1〜D-3 / D-8 を参照してください。
...
採否を決めてから、テストと実装を書く**という順序です。テスト名の先頭には仕様書の項目 ID
